...
export type ServerWebSocketHandlers = {
    ...
    message: ((websocket: HttpWebSocket, message: string | buffer) -> ())?,
...
export type HttpWebSocketOptions = {
    ...
    message: ((socket: HttpWebSocket, message: string | buffer) -> ())?,
    tls: {
        host: string?,
        ca: TlsCertBundle?, 
    }?, 
...
- `opts: HttpWebSocketOptions{:luau}` - The options for the WebSocket.
  ...
  - `message: ((socket: HttpWebSocket, message: string | buffer) -> ())?{:luau}` - The function to call when a message is received.
