...
This major release was long overdue and has been in development for a long time. The majority of the focus was on turning a poorly made runtime into a production-level runtime, with a lot of new features, improvements, and bug fixes.
...
Currently, the debugger is an interactive CLI similar to `gdb` program or `repl` command, but there is a [DAP server & VSCode extension](https://github.com/Sythivo/zune-debugger-vscode) built on top of this.
...
```sh
zune bundle src/main.luau "src/**/*.luau"
```
...
- Added `serde.flate` for flate compression and decompression.
...
### DateTime
Small improvements to the `datetime` library, such as more formatting and slight optimizations.
